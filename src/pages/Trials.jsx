...
export default function Trials({ onMenuClick }) {
  ...
  const filteredTrials = useMemo(() => {
    ...
    // Schwartzian transform: decorate-sort-undecorate for O(N) expensive parsing
    const ownUid = user?.uid || user?.ID || user?.id;
    const decoratedList = list.map(t => {
      const dateVal = new Date(t.Date || 0).getTime();
      const updatedTime = new Date(t.DateUpdatedAt || t.CreatedAt || t._createdAt?.toDate?.() || 0).getTime();
      ...
      return { t, dateVal, updatedTime, createdTime, obsCount, isShared };
    });
    ...
    decoratedList.sort((a, b) => {
      ...
      if (sortBy === 'date-asc') {
        ...
        if (dateDiff !== 0) return dateDiff;
        if (a.updatedTime !== b.updatedTime) return a.updatedTime - b.updatedTime;
